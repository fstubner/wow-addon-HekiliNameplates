...
-- Declared in the TOC as ## SavedVariables: HekiliNameplatesDB
...
local function initDB()
  ...
  -- Back-fill any new keys added in a later version
...
local savedPositions = {}  -- keyed by frameName
...
---Returns the best anchor frame + point on the given nameplate.
---Prefers the cast bar (when visible), falls back to the health bar,
---then to the nameplate base frame.
    ...
    if db().frames[info.key] then
      ...
      if hekiliFrame then
        ...
        if hasTarget then
          ...
          -- Centre the Hekili frame under the anchor (nameplate may report 0
          -- width if it hasn't laid out yet, so we guard against that)
          local npWidth = nameplate:GetWidth()
          local hkWidth = hekiliFrame:GetWidth()
          local centreOffX = (npWidth > 0 and hkWidth > 0)
              and ((npWidth - hkWidth) / 2)
              or  0

          hekiliFrame:ClearAllPoints()
          hekiliFrame:SetPoint(
            "TOP",
            anchor,
            anchorPoint,
            centreOffX + db().offsetX,
            db().offsetY
          )
        else
...
local function buildOptionsFrame()
  ...
  castBarCB:SetScript("OnClick", function(self)
    db().castBarAware = self:GetChecked()
    updatePositions()
  end)
  ...
  HNSliderXLow:SetText("-100")
  HNSliderXHigh:SetText("+100")
  HNSliderXText:SetText("X offset: " .. db().offsetX)
  xSlider:SetScript("OnValueChanged", function(self, val)
    local v = math.floor(val + 0.5)
    db().offsetX = v
    HNSliderXText:SetText("X offset: " .. v)
    updatePositions()
  end)
  f._xSlider = xSlider
  yOff = yOff - 42
  ...
  HNSliderYLow:SetText("-60")
  HNSliderYHigh:SetText("0")
  HNSliderYText:SetText("Y offset: " .. db().offsetY)
  ySlider:SetScript("OnValueChanged", function(self, val)
    local v = math.floor(val + 0.5)
    db().offsetY = v
    HNSliderYText:SetText("Y offset: " .. v)
    updatePositions()
  end)
  f._ySlider = ySlider
  yOff = yOff - 44
...
SlashCmdList["HEKILINAMEPLATES"] = function(msg)
  ...
  elseif cmd:match("^x%s+%-?%d+$") then
    local v = tonumber(cmd:match("%-?%d+"))  -- tonumber includes the sign
    db().offsetX = v
    updatePositions()
